...
class Matrix extends \Magento\Framework\View\Element\Template
{
    ...
    */
    ...
    public function __construct(
        ...
        \Magento\ConfigurableProduct\Model\Product\Type\VariationMatrix $configurableProductVariationMatrix,
                ...
                foreach ($confAttributes as $key => $confAttribute) {
                    if (isset($configurableAttributeData[$key])) {
                        $confAttributes[$key] = array_replace_recursive(
                            $confAttribute, $configurableAttributeData[$key]
                        );
    ...
    {
        $usedProductAttributes = $this->_configurableProductType->getUsedProductAttributes($this->getSellerProduct());
    ...
    protected function prepareConfigurableProductVariation()
    {
        ...
        if ($configurableProductVariations) {
            ...
            foreach ($configurableProductVariations as $configurableProductVariation) {
                $confAttributeValues = [];
                foreach ($usedProductAttributes as $confAttribute) {
                    $confAttributeValues[$confAttribute->getAttributeCode()] = $configurableProductVariation[
                        $confAttribute->getId()
                    ]['value'];
                }
                ...
                if (isset($productByUsedAttributes[$key])) {
                    ...
                    $configurableProductVariationOptions = array();
                    foreach ($usedProductAttributes as $confAttribute) {
                        ...
                        $confAttributes[$confAttributeId]['chosen'][$optionId] = $configurableProductVariationOption;
                    }
                    ...
                    $configurableProductMatrix[] = [
                        ...
                        'images' => [
                            'preview' => $this->_helperImage->init($product, 'product_thumbnail_image')->getUrl()
                        ],
                        'sku' => $product->getSku(),
