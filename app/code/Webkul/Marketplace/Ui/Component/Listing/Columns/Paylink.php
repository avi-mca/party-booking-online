    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                else if(($item['paid_status']==0||$item['paid_status']==4) && ($item['cpprostatus']==0)){
                    $item[$fieldName . '_html'] =__('Item Pending');
                }else if(($item['paid_status']==0 || $item['paid_status']==4 || $item['paid_status']==2) && ($item['cpprostatus']==1) && ($item['status']!='complete')){
                    $item[$fieldName . '_html'] =__('Item Pending');
                }else{
                    if($item['paid_status']==4){
                        $item[$fieldName . '_html'] =__('Item Cancelled');
                    }else if($item['paid_status']==3){
