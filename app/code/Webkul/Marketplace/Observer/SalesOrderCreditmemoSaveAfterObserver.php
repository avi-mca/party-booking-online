    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
            ...
            if ($item->getQty()) {
                foreach ($_items as $_ordereditem) {
                    ...
                    if ($availableSellerFlag == 1) {
                        ...
                        $_index = 0;
                        $_prevOptionId = '';
                        if ($_ordereditem->getParentItem()) continue;
                        if ($_ordereditem->getProductType()!='bundle') {
                            ...
                            foreach ($bundleitems as $_bundleitem) {
                                ...
                                if (isset($options['bundle_selection_attributes'])) {
                                    $attributesOption =  unserialize($options['bundle_selection_attributes']);
                                }
                                ...
                                if (!$_bundleitem->getParentItem()) {
                                    ...
                                    $creditmemoItemsPrice[$item->getOrderItemId()] + 
                                    $_bundleitem->getPrice()*intval($refundQtyArr[$_bundleitem['item_id']]);
                                }
                            }
                        }
                    }
        ...
        foreach ($creditmemoItemsPrice as $key => $item) {
            ...
            foreach ($sellerProducts as $sellerProduct) {
                ...
                if (
                    $sellerProduct['cpprostatus'] == Saleslist::PAID_STATUS_COMPLETE && 
        ...
        foreach ($sellerArr as $sellerId => $value) {
            ...
            foreach ($collectionverifyread as $verifyrow) {
                ...
                if (isset($sellerArr[$sellerId]['totalremain'])) {
                    ...
                    if (
                        isset($sellerArr[$sellerId]['is_shipping']) && 
